...
import TypeForm from "../../../components/settings/emptype/Form.vue";
import { VDataTable } from "vuetify/labs/VDataTable";

 const { types, pagination, query, is_loading, getTypes, destroyType } =     useTypes();
const type = ref({});
...
const updateType = async (row) => {
    type.value = row;
    show_form_modal.value = true;
};

const showTypeForm = async (is_show) => {
    type.value = {};
...
<template>
    <div style="text-align: end">
        <v-btn class="ma-2" color="blue-darken-1" @click="showTypeForm(true)">
    ...
    <v-card>
        ...
        >
            <v-card-title>List of Types </v-card-title>
